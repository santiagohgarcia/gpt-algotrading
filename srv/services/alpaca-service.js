...
class AlpacaService {
  ...
  async createBracketOrder(bracketOrder) {
    ...
    await this.api.createOrder({
      ...
      time_in_force: "gtc",
      take_profit: {
        limit_price: bracketOrder.takeProfitPrice.toFixed(2)
      },
      stop_loss: {
        stop_price: bracketOrder.stopLossPrice.toFixed(2)
        //limit_price: (bracketOrder.stopLossPrice * 0.98).toFixed(2)
      }
    });

    console.log(`ALPACA: Bracket Order Triggered for ${bracketOrder.symbol}. Side: ${bracketOrder.side}.`)

  }

  subscribeToNews(fnOnNews) {
    this.api.news_stream.onNews(fnOnNews)
  }
